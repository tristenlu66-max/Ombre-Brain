...
from desire_engine import DesireEngine, PARAMS as DESIRE_PARAMS  # Desire engine / 欲望引擎
desire_engine = DesireEngine(config)

# =============================================================
# v2.2 Execution Layer — Telegram narrow channels
# 执行层——Telegram 窄通道
#
# Two channels (agreed 2026-06-20):
#   attachment → whisper (碎语): pull recent memories, generate Evan-voice msg
#   curiosity → share (分享): search a topic, summarize, send to 鸿湍
#
# IO lives here (server.py), not in desire_engine.py.
# desire_engine only decides WHETHER to fire; this code does the firing.
# =============================================================
...
_EXEC_ENABLED = bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID
...
async def _exec_generate_message(intent: dict, memories: str) -> str:
...
async def _exec_get_recent_memories() -> str:
    """Pull recent unresolved memories for message generation context.
    拉最近未解决的记忆给LLM当上下文。复用/wakeup的逻辑。"""
    try:
...
async def _on_desire_tick(snapshot: dict) -> None:
    ...
    # 1. Pull context / 拉上下文
    memories = await _exec_get_recent_memories()

    # 2. Generate message / 生成消息
    message = await _exec_generate_message(intent, memories)
    if not message:
        logger.warning("Execution aborted: empty LLM response / 中止：LLM返回空")
        return

    # 3. Send via Telegram / 发Telegram
    success = await _telegram_send(message)
    if not success:
        return
    ...
    try:
        from desire_engine import BASELINE, marginal_gain
...
# Register the tick callback / 注册tick回调
